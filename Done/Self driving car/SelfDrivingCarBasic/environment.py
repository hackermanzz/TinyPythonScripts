...
class Environment(object):
    ...
    def parseConfigFile(self):
        ...
        while line:
                ...
                if linearr[0] == "FIX_MOVEMENT":
                        ...
                        for k in moves_str:
                                if k!="None":
                                    fixmovementlist.append(k)
                                else:
                                    fixmovementlist.append(None)
                if linearr[0] == "visibility":
                    visibility_range = int(linearr[1].strip())        
                line=f.readline()
        f.close()
    ...
    def __init__(self, verbose=False, num_dummies=20, grid_size = (8, 28),config_file="default_config.txt",fixmovement=False):
        ...
        self.goal_states=[]
        #for i in range(grid_size[0]):
        self.goal_states.append({"location":(grid_size[0]-4,grid_size[1]-2)})
        self.xmin= 0
    ...
    def interimstep(self):
        if self.primary_agent is not None:
            self.primary_agent.computePath()
    def step(self):
        ...
        for agent in self.agent_states.keys():
            if agent is not self.primary_agent:
                agent.update()
        #if len(self.optimalpath) > 1:
        #    self.optimalpath=self.optimalpath[1:]
        #else:
        self.optimalpath=[]
        self.t += 1
...
class DummyAgent(Agent):
    color_choices = ['cyan', 'red', 'blue', 'green', 'orange', 'magenta']
    #color_choices = ['orange']

    def __init__(self, env,location=None,fixmovement=False):
        super(DummyAgent, self).__init__(env,location) 
        loc_x=location[0]
        loc_y=location[1]
       # random.seed(loc_x*20+loc_y*30)
