...
class Simulator(object):
    ...
    def __init__(self, env, size=None, update_delay=2.0, display=False):
    ...
    def run(self):
        ...
        while True:
                try:
                    ...
                    # Update environment
                    if self.paused == False and self.env.done == False and (self.current_time - self.last_updated) >= 1:
                        self.env.interimstep()
                      #  print("interim",self.update_delay)
                    ...
                    if self.quit or self.env.done or self.env.t > 50:
                        break
    ...
    def render(self):
        ...
        if len(self.env.optimalpath) > 0:
                prev_offset=(self.env.block_size/2,self.env.block_size/2)
                initial_loc = (self.env.optimalpath[0][0],self.env.optimalpath[0][1])
                prev_loc = (self.env.optimalpath[0][0],self.env.optimalpath[0][1])
                print(initial_loc)
                prev_pos = ((self.env.optimalpath[0][0]+self.env.xadd )* self.env.block_size - prev_offset[0], (-self.env.optimalpath[0][1] -1+self.env.ymax+ self.env.yadd )* self.env.block_size - prev_offset[1])
                for k1 in range(1,len(self.env.optimalpath)):
                    k=self.env.optimalpath[k1]
                    agent_offset=(self.env.block_size/2,self.env.block_size/2)
                    agent_pos = ((k[0]+self.env.xadd )* self.env.block_size - agent_offset[0], (-k[1] -1+self.env.ymax + self.env.yadd )* self.env.block_size - agent_offset[1])
                    if math.fabs(k[0]-initial_loc[0]) < self.env.visibility_range and math.fabs(k[1]-initial_loc[1]) < self.env.visibility_range:
                    	self.pygame.draw.line(self.screen,self.colors['dgreen'], prev_pos,agent_pos,1)
                    else:
                        interim_pos=prev_pos
                        if k[0]-prev_loc[0] == 0  and k[1]-prev_loc[1] == 2:
                            if math.fabs(k[0]-initial_loc[0]) < self.env.visibility_range and math.fabs(k[1]-1-initial_loc[1]) < self.env.visibility_range:
                                interim_pos = ((k[0]+self.env.xadd )* self.env.block_size - agent_offset[0], (-k[1] + self.env.ymax + self.env.yadd )* self.env.block_size - agent_offset[1])
                                self.pygame.draw.line(self.screen,self.colors['dgreen'], prev_pos,interim_pos,1)
                        prev_pos = interim_pos
                        self.pygame.draw.line(self.screen,self.colors['red'], prev_pos,agent_pos,1)
                    prev_pos=agent_pos
                    prev_loc = k
        goalstate=self.env.getGoalState()
        goal_offset=(self.env.block_size/2,self.env.block_size/2)
        goal_pos = ((goalstate['location'][0]+self.env.xadd )* self.env.block_size -goal_offset[0], (-goalstate['location'][1] + self.env.ymax-1 + self.env.yadd )* self.env.block_size -goal_offset[1])
        tempsprite=self.pygame.transform.smoothscale(self.pygame.image.load(os.path.join("images", "goal.png")), self.primary_agent_sprite_size)
        goalspritesize = self.primary_agent_sprite_size
        self.screen.blit(tempsprite,
                    self.pygame.rect.Rect(goal_pos[0] - goalspritesize[0] / 2, goal_pos[1] - goalspritesize[1] / 2,
                        goalspritesize[0], goalspritesize[1]))
        # * Dynamic elements
